...
function isHeader(file: vscode.Uri): boolean {
  const headerExtensions = getConfig(file).get<string[]>("Header Extensions", [
    ".h",
    ".hpp",
    ".h++",
    ".hh",
  ]);
  return headerExtensions.some((headerExtension) =>
    file.fsPath.endsWith(headerExtension),
  );
}
...
function shouldUpdateGuard(file: vscode.Uri) {
  ...
  if (macroType !== "GUID") {
    // only effective when the macro has a filename or filepath component
...
function shouldAutoIncludeGuardInFile(file: vscode.Uri): boolean {
  ...
  const allowlist: string[] = getConfig(file).get<string[]>(
    "Auto Update Path Allowlist",
    [],
  );
  const blocklist: string[] = getConfig(file).get<string[]>(
    "Auto Update Path Blocklist",
    [],
  );
  ...
  const relativePath = file
    .toString()
    .substring(baseUri.uri.toString().length + 1);
  if (
    allowlist.length === 0 ||
    allowlist.some((path) => relativePath.startsWith(path))
  ) {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  vscode.workspace.onDidCreateFiles(async (event) => {
    for (const newFile of event.files) {
      if (
        getConfig(newFile).get<boolean>(
          "Auto Include Guard Insertion For New File",
          true,
        )
      ) {
        if (shouldAutoIncludeGuardInFile(newFile) && isHeader(newFile)) {
          vscode.workspace
            .openTextDocument(newFile)
            .then((doc) =>
              vscode.window
                .showTextDocument(doc)
                .then(commands.insertIncludeGuard),
            );
        }
      }
    }
  ...
  vscode.workspace.onDidRenameFiles((event) => {
    for (const renamedFile of event.files) {
      if (shouldUpdateGuard(renamedFile.newUri)) {
        if (
          isHeader(renamedFile.newUri) &&
          isOpenedInEditor(renamedFile.newUri)
        )
  ...
  vscode.workspace.onDidOpenTextDocument(async (document) => {
    if (shouldUpdateGuard(document.uri) && isHeader(document.uri)) {
      await commands.updateIncludeGuard(false); //^ Do not insert include guard when there is none found
    }
  });
}
